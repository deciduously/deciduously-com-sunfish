...
impl BlogPost {
    ...
    fn write_template(&self) -> Result<(), io::Error> {
        ...
        writeln!(file, "{{% block content %}}<main>{}</main>{{% endblock %}}", html)?;
        Ok(())
    }
    fn write_template_struct(&self, file: &mut fs::File) -> Result<(), io::Error> {
        let struct_name = format!("Blog{}Template", self.id);
        ...
        writeln!(file, "}}")?;
        writeln!(file, "impl Default for {} {{", &struct_name)?;
...
fn generate_module(blog: &Blog) -> Result<(), io::Error> {
    ...
    writeln!(module, "// this module was auto-generated by build.rs")?;
    writeln!(module, "use askama::Template;")?;
    writeln!(module, "use crate::{{config::NAV, types::Hyperlink}};")?;
